...
export function StoreProvider({ children }) {
  ...
  useEffect(() => {
    ...
    return () => { unsubInv(); unsubBills(); unsubVeg(); unsubMilk(); unsubProfile(); unsubCategories(); };
  }, []);
  ...
  const addInventoryItem = useCallback(async (name, altName, price, unit, category) => {
    ...
    if (numericPrice > 0 && !isNaN(numericPrice)) data.price = numericPrice;
  ...
  const updateCategories = useCallback(async (nextCategories) => {
    const clean = nextCategories.filter((category) => category?.id && category?.label);
    await setDoc(doc(db, 'config', 'categories'), { categories: clean }, { merge: true });
    setCategories(clean);
  }, []);
