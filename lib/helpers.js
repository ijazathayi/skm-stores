...
// ── billing total: always round fractions up to the next whole rupee ──
export const roundToRupee = (n) => Math.ceil((Number(n) || 0) - Number.EPSILON);

// ── amount added or deducted to reach the rounded bill total ──
export const roundOffAmount = (n) => {
  const amount = Number(n) || 0;
  return Math.round((roundToRupee(amount) - amount) * 100) / 100;
};
...
export const STORE_CATEGORIES = [
  ...
  { id: 'TL', prefix: 'TL', label: 'Tailoring',          labelTa: 'தையல்',                    icon: '🧵',
    keywords: ['tailoring', 'tailor', 'stitching', 'alteration', 'blouse stitching', 'pant stitching', 'shirt stitching', 'தையல்'] },
  { id: 'FN', prefix: 'FN', label: 'Fancy',              labelTa: 'ஃபேன்சி',                  icon: '🎀',
    keywords: ['fancy', 'hair clip', 'hair pin', 'hair band', 'bangle', 'bindi', 'comb', 'ribbon'] },
  ...
  { id: 'PL', prefix: 'PL', label: 'Plastic Items',     labelTa: 'பிளாஸ்டிக் பொருட்கள்',       icon: '🧺',
    keywords: ['plastic', 'bucket', 'mug', 'container', 'cover', 'bag'] },
  { id: 'OT', prefix: 'OT', label: 'Other',              labelTa: 'மற்றவை',                  icon: '📦',
    keywords: [] },
];

export function getProductCategory(item, categories = STORE_CATEGORIES) {
  if (item?.category && item.category !== 'auto' && item.category !== 'all') {
    if (categories.find((c) => c.id === item.category)) return item.category;
    if (/^[A-Z][A-Z0-9_-]{1,15}$/.test(String(item.category))) return item.category;
  }
  if (item?.productId) {
    const pid = String(item.productId).trim();
    const m = pid.match(/^([A-Z]{2})\d+$/);
    if (m) {
      const cat = categories.find((c) => c.prefix === m[1]);
...
export function generateProductId(categoryId, existingIds = [], categories = STORE_CATEGORIES) {
  const cat = categories.find((c) => c.id === categoryId);
  const prefix = cat?.prefix || String(categoryId || 'OT').replace(/[^A-Z0-9]/gi, '').slice(0, 2).toUpperCase() || 'OT';
