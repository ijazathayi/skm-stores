...
import { STORE_CATEGORIES, getProductCategory } from './helpers';
...
export async function parseImportFile(file, inventory) {
  ...
  if (iProductId === -1 || iName === -1) {
    throw new Error('Could not find "Product ID" or "Name" columns. Make sure you are uploading the exported SKM file.');
  }

  for (let i = headerIdx + 1; i < raw.length; i++) {
    const row = raw[i];
    const pid = String(row[iProductId] || '').trim().toUpperCase();
    ...
    if (!pid) { skipped++; continue; } // product ID is mandatory for upsert
    ...
    const category = (catMatch || catFromId)?.id || 'OT';
    ...
    if (existingByPid[pid]) {
      ...
      toUpdate.push({ firestoreId: existingByPid[pid].id, productId: pid, updates });
    } else {
      // New product — add
      toAdd.push({ productId: pid, ...updates });
    }
  }
