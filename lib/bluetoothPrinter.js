...
function receiptText(bill, profile = {}) {
  ...
  return [
    '\x1b\x61\x01', profile.storeName || 'SKM STORES', '\n',
...
export async function printReceiptBluetooth(bill, profile = {}) {
  ...
  const text = new TextEncoder().encode(receiptText(bill, profile));
  const logo = await logoRaster();
  const prefix = new Uint8Array([0x1b, 0x40, 0x1b, 0x61, 0x01]);
  const suffix = new Uint8Array([0x0a, 0x0a, 0x0a, 0x1d, 0x56, 0x00]);
