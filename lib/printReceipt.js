...
export function getReceiptHTML(bill, profile = {}, options = {}) {
  ...
  const lang = options.lang || profile.printLang || (typeof window !== 'undefined' ? localStorage.getItem('skm_printLang') || 'en' : 'en');
  const isTa = lang === 'ta';
...
<body>
  <div class="logo-wrap">
    <img src="/skm-logo-black.png" class="logo" alt="SKM" onerror="this.src='/skm-logo.png'; this.style.filter='grayscale(100%) contrast(300%)';" />
  </div>
  ...
  <table>
    ...
    <tbody>
      ${(bill.items || []).map((it, idx) => {
        const itemName = isTa && it.altName ? it.altName : it.name;
        return `
        <tr>
