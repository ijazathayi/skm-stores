...
async function tamilReceiptRaster(bill, profile) {
  ...
  container.innerHTML = getReceiptHTML(bill, profile, { width: 58, lang: 'ta' });
  ...
  body.style.fontFamily = 'Noto Sans Tamil, Latha, sans-serif';
  body.querySelectorAll('.title').forEach((element) => { element.style.fontSize = '25px'; });
  body.querySelectorAll('.sub').forEach((element) => { element.style.fontSize = '16px'; });
  body.querySelectorAll('.tag').forEach((element) => { element.style.fontSize = '20px'; });
  body.querySelectorAll('.meta').forEach((element) => { element.style.fontSize = '17px'; });
  body.querySelectorAll('table').forEach((element) => { element.style.fontSize = '17px'; });
  body.querySelectorAll('th').forEach((element) => { element.style.fontSize = '16px'; });
  body.querySelectorAll('.total-row').forEach((element) => { element.style.fontSize = '21px'; });
  body.querySelectorAll('.thanks').forEach((element) => { element.style.fontSize = '18px'; });
  body.querySelectorAll('.logo').forEach((element) => { element.style.width = '84px'; element.style.height = '84px'; });
  document.body.appendChild(container);

  try {
    const canvas = await html2canvas(body, { backgroundColor: '#fff', scale: 1, width: 384, useCORS: true });
    return canvasToRasterCommand(canvas);
  } finally {
    container.remove();
  }
