...
async function connectToPrinter(device) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      return { server, characteristic: await findWritableCharacteristic(server) };
    } catch (error) {
      lastError = error;
      if (device.gatt.connected) device.gatt.disconnect();
  ...
  throw new Error(`Connection failed after retry: ${lastError?.message || 'Bluetooth device unavailable'}`);
}
...
export async function printReceiptBluetooth(bill, profile = {}, options = {}) {
  ...
  const suffix = new Uint8Array([0x0a, 0x0a, 0x0a, 0x0a, 0x1d, 0x56, 0x00]);
